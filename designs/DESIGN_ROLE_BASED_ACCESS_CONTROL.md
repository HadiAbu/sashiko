...
In `../bugrepo`, authorization was modeled with a rudimentary single-level JWT mechanism: an email was verified through a sign-in link, and an authenticated user had global access unless authorization was disabled completely (`disable_auth = true`).
...
1. **Interactive Session Tokens (JWT)**: Used by web browser dashboards via sign-in link email verification or OIDC/OAuth providers.
